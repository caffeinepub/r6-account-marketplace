    ...
    <div className="container mx-auto px-4 py-10 max-w-4xl">
      ...
      <div className="mb-8 space-y-2">
        ...
        <p className="text-muted-foreground">
          Verified reviews from real buyers. All reviews are tied to confirmed
          purchases.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-4">
          {(["sk-a", "sk-b", "sk-c", "sk-d", "sk-e"] as const).map((k) => (
            <Skeleton key={k} className="h-28 bg-surface-raised rounded-sm" />
          ))}
        </div>
      ) : !vouches || vouches.length === 0 ? (
        ...
        <div className="space-y-4">
          {vouches.map((vouch) => (
            <VouchCard
              key={String(vouch.timestamp) + vouch.anonymousUsername}
              vouch={vouch}
            />
          ))}
        </div>

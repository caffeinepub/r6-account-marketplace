...
import { RANK_COLORS, RANK_ICONS } from "../../lib/marketplace-utils";
...
export default function VouchPromptModal({
  purchaseId,
  rank,
  onClose,
}: VouchPromptModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        result ===
        Variant_notAuthenticated_notPurchased_alreadyVouched_success_invalidRating.success
      ) {
        setSubmitted(true);
        toast.success("Review submitted! Thank you.");
      } else if (
        result ===
        Variant_notAuthenticated_notPurchased_alreadyVouched_success_invalidRating.alreadyVouched
      ) {
        toast.info("You have already submitted a review for this purchase.");
        onClose();
      } else if (
        result ===
        Variant_notAuthenticated_notPurchased_alreadyVouched_success_invalidRating.notAuthenticated
      ) {
        toast.error("You must be logged in to submit a review.");
      } else if (
        result ===
        Variant_notAuthenticated_notPurchased_alreadyVouched_success_invalidRating.notPurchased
      ) {
        toast.error("No confirmed purchase found for this review.");
      } else if (
        result ===
        Variant_notAuthenticated_notPurchased_alreadyVouched_success_invalidRating.invalidRating
      ) {
        ...
        {submitted ? (
          <div className="text-center py-6 space-y-3 animate-slide-up">
            ...
            <p className="text-sm text-muted-foreground">
              Your review has been submitted and will be visible publicly.
            </p>
            <Button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-sm border text-sm font-semibold ${rankColor}`}
              >
                <span>{rankIcon}</span>
                <span>{rank}</span>
              </span>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <Button
                type="submit"
                disabled={
                  !username.trim() || rating === 0 || submitVouch.isPending
                }

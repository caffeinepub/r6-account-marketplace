    ...
    <div className="p-4 bg-surface border border-border rounded-sm hover:border-gold/30 transition-all space-y-3 animate-slide-up">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <p className="font-semibold text-foreground text-sm">{vouch.anonymousUsername}</p>
          <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-sm border text-xs font-semibold ${rankColor}`}>
            <span>{rankIcon}</span>
        ...
        <div className="text-right space-y-1">
          ...
          <p className="text-xs text-muted-foreground">{formatTimestamp(vouch.timestamp)}</p>
        </div>
      </div>

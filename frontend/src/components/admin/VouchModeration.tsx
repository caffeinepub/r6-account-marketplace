...
import { useGetMyVouches, useHideVouch, useDeleteVouch } from '../../hooks/useQueries';
import { formatTimestamp, RANK_COLORS, RANK_ICONS } from '../../lib/utils';
...
export default function VouchModeration() {
  ...
  const handleDelete = async (vouchId: string) => {
    if (!confirm('Delete this vouch permanently? This cannot be undone.')) return;
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-xl font-bold text-foreground">Review Moderation</h2>
        <Button
          variant="outline"
          size="sm"
          onClick={() => refetch()}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border hover:bg-transparent">
                ...
                <TableHead className="text-muted-foreground text-xs">Review</TableHead>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button
                          ...
                          onClick={() => handleHide(vouch.vouchId, vouch.hidden)}
